...
\def\XX{\chi}
\def\degmax{\underset{\text{max}}{\deg}}
\def\degmin{\underset{\text{min}}{\deg}}
\def\Lagr{\mathcal{L}}
\def\ex#1{\mathrm{e}^{#1}}
\def\i{\mathrm{i}\,}
...
\newcommand{\al}[1]{{\am^{(#1)}}^{\mu}}
\newcommand{\sal}[3]{{\am^{(#1)}_{#2, #3}}^{\mu}}
\newcommand{\nl}{n_l}
...
% handle footnote numbering starting at 2
% https://tex.stackexchange.com/questions/359253/why-does-starting-a-new-chapter-makes-footnotes-start-at-2
\usepackage{remreset}
\makeatletter
...
\usepackage[compat=1.1.1]{tikz-feynman}
...
\tikzset{cross/.style={cross out, draw=black, minimum size=2*(#1-\pgflinewidth), inner sep=0pt, outer sep=0pt},cross/.default={5pt}}

%\usepackage{tikzfeynhand}
\newcommand{\ddd}[7]{
    \vertex [dot, style={/tikz/minimum size=0.05cm}] [above right=(#2)*sin(#6   )/2 and (#2)*cos(#6   )/2 of #1] (#3) {};
    \vertex [dot, style={/tikz/minimum size=0.05cm}] [above right=(#2)*sin(#6+#7)/2 and (#2)*cos(#6+#7)/2 of #1] (#4) {};
    \vertex [dot, style={/tikz/minimum size=0.05cm}] [above right=(#2)*sin(#6-#7)/2 and (#2)*cos(#6-#7)/2 of #1] (#5) {};
}
