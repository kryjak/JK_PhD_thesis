...
\begin{equation} \label{eq:kdecomposition1Lbox}
    k_1^{(D)} = \alpha_{11} p_1 + \alpha_{12} p_2 + \alpha_{13} p_3 + \beta_1 \omega + k_1^{(-2\eps)}\,.
\end{equation}
Taking the scalar products of this equation with $p_1, p_2, p_3$ and $\omega$, we find the following parameters $\alpha_i$ and $\beta$ \JK{Improve formatting}:
\begin{align}
    \alpha_{11} &= -\frac{D_4}{4 p_1 \cdot p_3} + \frac{D_3 (p_1\cdot p_2 - p_1 \cdot p_3)}{4 p_1 \cdot p_2 p_1 \cdot p_3} - \frac{D_1 p_2 \cdot p_3}{4 p_1 \cdot p_2 p_1 \cdot p_3} + \frac{D_2 (p_1\cdot p_3 + p_2 \cdot p_3)}{4 p_1 \cdot p_2 p_1 \cdot p_3} + \frac{p_2 \cdot p_3}{2 p_1 \cdot p_3} + 1\,, \nonumber \\
    \alpha_{12} &= -\frac{D_4}{4 p_2 \cdot p_3} + \frac{D_1}{4 p_1 \cdot p_2} + \frac{D_3 (p_1\cdot p_2 + p_1 \cdot p_3)}{4 p_1 \cdot p_2 p_2 \cdot p_3} - \frac{D_2 (p_1\cdot p_3 + p_2 \cdot p_3)}{4 p_1 \cdot p_2 p_2 \cdot p_3} + \frac{1}{2}\,, \nonumber \\
    \alpha_{13} &=  \frac{D_1}{4 p_1 \cdot p_3} + \frac{D_4 p_1 \cdot p_2}{4 p_1 \cdot p_3 p_2 \cdot p_3} - \frac{D_3 (p_1\cdot p_2 + p_1 \cdot p_3)}{4 p_1 \cdot p_3 p_2 \cdot p_3} + \frac{D_2 (p_1\cdot p_3 - p_2 \cdot p_3)}{4 p_1 \cdot p_3 p_2 \cdot p_3} -\frac{p_1 \cdot p_2}{2 p_1 \cdot p_3}\,, \nonumber \\
    \beta &= \frac{k_1^{(4),\, \omega} \cdot \omega}{\omega^2}\,.
\end{align}
We can then substitute the parametrisation into all the monomials appearing in Eq.~\ref{eq:ampschematic}. For example:
\begin{align}
    \bra{1}k_1^{(D)}\ketsq{2} &= \sum_{a=1}^3 \alpha_a \bra{1}p_a\ketsq{2} \nonumber \\
    &= \alpha_3 \braket{13}\braketsq{32} \JK{check sign}\,,
\end{align}
where the first line follows from the orthogonality properties of the loop momentum decomposition, while in the second we have imposed the massless Weyl equation~\ref{eq:weyleq}. We then collect the result in $D_t$, whose coefficients are given by rational functions of external kinematics only. As another example, the square of the loop momentum will contain the spurious and $(-2\eps)$ terms:
\begin{equation}
    k_1^{(D)} \cdot k_1^{(D)} \ni \frac{(k_1^{(4),\, \omega} \cdot \omega)^2}{\omega^2} - \mu_{11}\,.
\end{equation}
Most generally, this substitution will also contain the physical ISPs, which will appear provided the condition in Eq.~\ref{eq:ISPcondition} is satisfied.

Typically, we want to avoid performing the substitution analytically due to the high rank of monomials in Eq.~\ref{eq:ampschematic} and the complexity of their coefficients. Fortunately, it can also be performed numerically as an additional step in our finite field workflow. To this end, 
\textcolor{gray}{
\begin{itemize}
