...
\begin{equation} \label{eq:ampafterIBP}
    	A_n^{(L)} \left(1^{h_1}, 2^{h_2}, \ldots, n^{h_n} \right) =  
     \sum_{i=1}^{|\text{MI}|} g_i(p(\mathbf{x}), \varepsilon) \times \text{MI}_i(k,p,\varepsilon)\,,
\end{equation}
...
\end{equation}
That is, the number of MIs for all families considered together is not the same as the sum of numbers of MIs for these families considered separately. This statement might seem surprising. Indeed, if we formulate one big system of IBP relations between integrals in all the families and perform a pure IBP reduction, the resultant MI basis is equal to the sum of the bases obtained by solving smaller systems of IBP relations within each family one by one. This is because integrals in different families live in different vector spaces and the IBP relations, by their very nature, cannot find inter-family relations. Nonetheless, other kinds of relations exist that can help us further reduce the number of linearly independent integrals (for details, see Refs. \cite{Grozin:2011mt, Lee:2008tj, Lee:2012cn}). They are typically appended to the `pure' IBP system and make it less under-determined. In particular, integrals belonging to sectors other than the top sector can often be mapped between families. We present an easy to understand example in Fig.~\ref{fig:symmetryrelation}. We will return to the observation of Eq.~\ref{eq:noofmis} in Sec.~\ref{sec:altibps}.
\begin{figure}
    \centering
...
\end{equation}
They are related to the usual logarithms through:
\begin{equation}
...
%Summary of MI work for the QCD processes relevant to us
%Insert references to show where the dlog DEs have been used to calculate MIs

\subsection{Uniform transcendentality} \label{sec:UT}
When talking about canonical DEs, it is also useful to introduce the idea of `transcendentality'. For an iterated integral $f$, its \textbf{transcendental weight} $\mathcal{T}$ is simply the count of iterated integrations needed to define $f$. For example, $\mathcal{T}(\log x) = 1$, while $\mathcal{T}(G_n(a_1, \ldots, a_n;\, x)) = n$. From the definition, it follows that $\mathcal{T}(f_1 f_2) = \mathcal{T}(f_1) + \mathcal{T}(f_2)$, but note that $\mathcal{T}(f_1 + f_2)$ cannot be defined unless $\mathcal{T}(f_1) = \mathcal{T}(f_2)$. furthermore, algebraic factors have weight 0. Constants which can be obtained as special values of iterated integrals have the corresponding weight, e.g. $\mathcal{T}(\pi) = 1$, since $\log (-1) = \pm \i \pi$ and $\mathcal{T}(i) = \mathcal{T}(-1) = 0$. \JK{Why is $\mathcal{T}(\zeta(n)) = n$?}

$\mathcal{T}(\varepsilon) = -1$
%pure integrals
%Why UT is important
%conjecture about the weight of special functions required for amplitudes
...
In practice, these missing relations between MIs can be obtained by performing their reduction in the traditional approach\footnote{Note that this reduction will not rely on identities of the IBP nature, as inter-family IBP relations do not exist. Instead, it will use Lorentz invariance identities, symmetries between propagators, etc. See Refs.~\cite{Grozin:2011mt, Lee:2012cn} for details.}, which expresses the MIs from all families in terms of the truly independent ones:
\begin{equation}
