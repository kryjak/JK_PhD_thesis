...
\section{The $\dd \log$ form and Goncharov Polylogarithms}
...
\textbf{Example 3}
\newline
For more complicated identities, calculating the symbol by hand becomes unfeasible. Consider the following wei relation:
\begin{align}
...
The \texttt{PolyLogTools} package introduces several powerful tools related to the symbol.

Moreover, it is also possible to `integrate' the symbol, that is to find a linear combination of GPLs which has the same symbol. This means we can determine the functional form corresponding to a symbol expressions --- up to an overall constant. For details of the procedure, see Refs.~\cite{Duhr:2011zq}and \cite{Duhr:2019tlz}.


\subsection{Relation to discontinuities}
