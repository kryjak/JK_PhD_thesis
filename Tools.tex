...
The choice of the colour basis is not unique. We adopt the decomposition according to traces of the $SU(N_c)$ generators in the fundamental representation. As an example, let's look at 4-gluon scattering at tree-level\JK{Redo this diagram using tikzfeynman}:
...
\begin{enumerate}
    \item Fill the twistor matrix $Z_i^I, i=1, \ldots, n$ with random integers\JK{Shouldn't this be `rational numbers'?}.
...
%Consider the 1L box. It has the following propagators:
%\begin{equation}
%    \{k_1^2, (k_1-p_1)^2, (k_1-p_1-p_2)^2, (k_1+p_4)^2 \} \,.
%\end{equation}
\JK{Quote ~\cite{Ellis:2011cr, Mastrolia:2016dhn, badger:2017jhb} somewhere.}
...
\begin{equation} \label{eq:4dextradimspace}
    k^{(D)} = k^{(4)} + k^{(-2\eps)}\,.
\end{equation}
Here and below, we have suppressed the Lorentz index for readability. Let us focus on the $4$-dimensional part first. We would like it to be spanned by the $4$-dimensional external momenta. However, due to momentum conservation, in an $n$-particle scattering process, only $n-1$ momenta can be linearly independent. Thus, they can span only an $(n-1)$-dimensional subspace, which we refer to as the `physical' space (labelled using $\parallel$). For $n\le4$, the remainder of the $4$-dimensional space needs to be completed by constructing $4-(n-1) = 5-n$ `spurious' vectors (labelled using $\omega$), which can be chosen as orthonormal to each other and also orthogonal to vectors in the physical space. Therefore, the decomposition in Eq.\ref{eq:4dextradimspace} becomes:
...
\end{equation}
Overall, the explicit decomposition of each loop momentum $k_i^{(D)}$ in terms of basis vectors is:
\begin{equation} \label{eq:kdecomposition}
    k_i^{(D)} = \underbrace{\sum_{a=1}^{n-1} \alpha_{ia} p_a}_{k^{(4),\,\parallel}} + \underbrace{\sum_{b=n}^{4} \alpha_{ib} w_b}_{k^{(4),\,\omega}} + \underbrace{\beta_i \mu}_{k^{(-2\eps)}}\,,
\end{equation}
where $p_a$ are the linearly indepdendent external momenta, $w_b$ are the vectors spanning the spurious space and $\mu$ is a vector spanning the extra-dimensional space \JK{Should it be $\mu$ or $\mu_i$ (separate for each $k_i$)?}\JK{How do we construct $\omega$? Using Levi-Civita? What about $\mu$ - does it have an explicit representation?}. If $n>4$, the second sum is not present since there is no spurious space. 

To find the parameters $\alpha$ and $\beta$, we take the scalar products of Eq.~\ref{eq:kdecomposition} with external momenta. On the LHS, we express the products\footnote{Since we work in the HV scheme and the spurious space is orthogonal to the physical one, $k_i^{(D)} \cdot p_j = k_i^{(4)} \cdot p_j = k_i^{(4),\,\parallel} \cdot p_j$.} $k_i^{(D)} \cdot p_j$ in terms of the inverse propagators $D_t(k,p)$ belonging to the maximal topologies we have mapped our integral onto (see Eq.~\ref{eq:ampschematic}). It turns out, however, that in certain cases not all such scalar products can be expressed through the inverse propagators. The remaining products are then referred to as irreducible scalar products (ISPs). More precisely, a simple counting argument shows that this happens when~\cite{grozin:2011mt}:
\begin{equation}
    L \ge \max (2, 7-2n)\,,
\end{equation}
...
Returning back to Eq.~\ref{eq:kdecomposition}, taking its scalar products with external momenta produces a system of equations linear in $\alpha$ and $\beta$, with the LHS given by linear combinations of the inverse propagators and (potentially) ISPs. This system can be easily inverted using the finite field framework. The coefficients $\alpha_a$ in the physical space are then functions of inverse propagators and ISPs, while the coefficients $\alpha_b$ and $\beta$ in the transverse space are functions of the `transverse' ISPs $k_i \cdot w_j$ and $\mu_{ij} \equiv k_i^{(-2\eps)} \cdot k_j^{(-2\eps)}$, respectively. 
\begin{itemize}
...
\end{equation}
where $\vec{0}_m$ denotes a series\JK{vector?} of zeros of length $m$. For example, $\Li_{2,2}(x, y)$ in the weight-4 basis Eq.~\ref{eq:weight4basis} can be written as:%admits the integral representation:
\begin{equation}
...
We are now ready to take on the computation of scattering amplitudes at the cutting edge of current knowledge. In the subsequent chapters, we will use the workflow developed here to tackle two-loop amplitudes for two processes directly relevant to LHC phenomenology, $\ppbbh$ and $\ppWgj$.
\end{document}