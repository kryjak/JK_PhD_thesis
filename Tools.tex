...
\JK{I don't understand why 1712.02229 and 1608.01902 talk about constructing a monomial basis and then mapping the numerator onto it. Once we've parametrised each $k_i$, why can't we just substitute it into the numerator? The denominators $D$ will cancel out and we'll have scalar products $p_i \cdot p_j$, as well as $k_i \cdot w_j$ and $\mu_{ij}$, which can be integrated out.}
...
%This form is usually the most compact one. 
...
\end{equation}
where $\vec{0}_m$ denotes a vector of zeros of length $m$. For example, $\Li_{2,2}(x, y)$ in the weight-4 basis Eq.~\ref{eq:weight4basis} can be written as:%admits the integral representation:
\begin{equation}
...
Since CIIs are defined as repeated integrations over some kernels\JK{Defined through Eq.~\ref{eq:MIlaurentexp}?}, the total differential acts on the symbol defined through Eq.~\ref{eq:symboldef} in the following way:
