...
\begin{equation}
    	A_n^{(L)} =  \sum_{T\in topologies} \left[\, \prod_{j=1}^{L} \left(\int \mathrm{d}^d k_j \right) \frac{\sum_i c_i(p(\mathbf{x}))m_i(k,p)}{\prod_{t\in T} D_t(k,p)} \right]\,.
\end{equation}
The main sum runs over distinct integral topologies, that is sets of (inverse) propagators $D_t$ associated with a given Feynman diagram. Each propagator depends on external momenta $p$, as well as one or more loop momenta $k$, which act as the integration variables for the $d$-dimensional integrals (see Sec.~\ref{sec:divergences}. In the numerator of each topology, we have a sum of monomials of both loop and external momenta, which are multiplied by coefficients that depend on external momenta \textit{only}. The monomials are objects such as scalar products and spinor brackets, e.g. $\left(k_i \cdot p_3 \right)^2, \braket{ij}$
In the following, we will specialise to the case $L=2$ since we are interested in computing two-loop amplitudes.
