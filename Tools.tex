...
As an example, consider the one-loop box diagram with massless propagators and external momenta, Fig.~\ref{fig:1Lboxnodot}. It has the following inverse propagators $D_t$:
...
\end{equation}
Taking the scalar products of this equation with $p_1, p_2, p_3$ and $\omega$, we find the following parameters $\alpha_{1i}$ and $\beta$ \JK{Improve formatting}:
\begin{align}
...
\end{align}
\textcolor{gray}{Maybe this formatting is better:
\begin{align}
    \alpha_{11} &= -\frac{D_4}{4 p_1 \cdot p_3} + \frac{D_3 (p_1\cdot p_2 - p_1 \cdot p_3)}{4 (p_1 \cdot p_2) (p_1 \cdot p_3)} - \frac{D_1 (p_2 \cdot p_3)}{4 (p_1 \cdot p_2) (p_1 \cdot p_3)} + \nonumber \\ 
    & \hspace{6.8cm} \frac{D_2 (p_1\cdot p_3 + p_2 \cdot p_3)}{4 (p_1 \cdot p_2) (p_1 \cdot p_3)} + \frac{p_2 \cdot p_3}{2 p_1 \cdot p_3} + 1\,, \nonumber \\
    & \; \; \vdots \nonumber \\
    \beta &= \frac{k_1^{(4),\, \omega} \cdot \omega}{\omega^2}\,.
\end{align}}
...
In summary, the numerators of the Feynman integrals will contain the following four types of terms after we substitute the parametrisation of Eq.~\ref{eq:kdecomposition} into them: the inverse propagators $D_t$, the physical ISPs, the spurious ISPs and the extra-dimensional scalar products $\mu_{ij}$. Crucially, the last two types can be eliminated through transverse integration. Specifically, the spurious ISPs either lead to vanishing integrals or can be traded for terms of the other three types. The $\mu_{ij}$ terms on the other hand can be integrated directly. They can be expressed as integrals of the same kinematic configuration, but in higher dimensions~\cite{Bern:1995db}. This can improve their IR behaviour, often leading to integrals which are finite or even vanish in the limit $\eps \to 0$. For more information about transverse integration, we refer the reader to Section~3.4 and~3.5 of Ref.~\cite{Badger:2023eqz}.\JK{I don't fully understand this. Some $\mu_{ij}$ integrals are rational + $\order{\eps}$, but some are not. For example, the pentagon integral with $\mu^2_{ij}$ will lead to an 8D scalar pentagon, which (I think) is UV-divergent. So what do we do with such integrals?}

Overall, we are left with only the first two types of terms in the numerators: the inverse propagators $D_t$ associated with the given maximal topology we map our integral onto, as well as the physical ISPs needed to cover any possible scalar product $k_i \cdot p_j$ at this loop order and multiplicity. This completes an integral family. The $D_t$ terms will of course cancel out with their counterparts in the denominators. What remains are either purely scalar integrals, i.e. with numerators equal to $1$, or integrals with numerators built from (monomials of) the physical ISPs. Any coefficients of such terms will depend on the external kinematics only and can be factored outside of the integrals. Once again, we remark that due to our rational parametrisation of external momenta, all the steps described in this section can be seamlessly integrated within the finite-field workflow.
