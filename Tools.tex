...
\begin{equation} \label{eq:ratfun}
     f(\mathbf{x}) = \frac{
     \sum_{\bm{\alpha}} n_{\bm{\alpha}} \mathbf{x}^{\bm{\alpha}}
     }{
     \sum_{\bm{\alpha}} d_{\bm{\alpha}} \mathbf{x}^{\bm{\alpha}}
     }\,,
\end{equation}
with $n_{\bm{\alpha}}, n_{\bm{\alpha}} \in \mathbb{Z}_p$. Here, $\mathbf{x}^\alpha$ are the multi-variate monomials:
\begin{equation}
...
\begin{equation}
    \deg (\mathbf{x}^{\bm{\alpha}}) = \sum_{i=1}^n \alpha_i\,.
\end{equation}
...
With this representation of $f(\mathbf{x})$ in Eq.~\ref{eq:ratfun}, we can try to reconstruct its analytic dependence from the numerical samples over the prime field $\mathbb{Z}_p$. In a very naive approach, we would construct the most general ansatz that would cover all possible monomials up to degree $\degmax (f(\mathbf{x}))$. We make this ansatz $R(\mathbf{x})$ for a rational function explicit by writing:
\begin{equation}
    R(\mathbf{x}) = \frac{
    \sum\limits_{i=0}^u \sum\limits_{\bm{\alpha}:\, |\bm{\alpha}| = i} n_{\bm{\alpha}} \mathbf{x}^{\bm{\alpha}}
    }{
    \sum\limits_{i=0}^v \sum\limits_{\bm{\alpha}:\, |\bm{\alpha}| = i} d_{\bm{\alpha}} \mathbf{x}^{\bm{\alpha}}
    }\,,
\end{equation}
where we have abbreviated the numerator/denominator degrees as $u=\degmax(\text{num}(f))$ and $v=\degmax(\text{den}(f))$. This is far from optimal, however, since such a general ansatz grows rapidly with both the degree as well as the number of variables. In fact, one can show the number of terms present is
