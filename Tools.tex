...
\begin{table}[b]
	\begin{center}
		...
		\end{tabular}
\end{center}
\caption{Selected works relevant to the computation of two-loop, five-point pure master integral bases. All propagators are massless. \JK{Insert reference for double-pentagons.}}
\label{tab:MIs}
\end{table}
...
We make two final remarks. First, we emphasise that the finite remainder is dependent on both the renormalisation and regularisation schemes. It is important to bear that in mind when cross-checking results or trying to restore the full amplitude from its remainder. Second, the pole subtraction in Eq.~\ref{eq:finrem} can be straightforwardly implemented over finite fields as a subtraction on the monomial coefficients $q_i$ in Eq.~\ref{eq:ampexpandedspfuncs}. At long last, our object of interest, the $n$-particle, $L$-loop finite remainder is written as:
\begin{equation}
