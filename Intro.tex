...
\end{equation}
with $T_F$ typically $\frac{1}{2}$ or $1$. Finally, let us list one more extremely useful identity satisfied by the fundamental generators:
\begin{equation} \label{eq:fierzcolour}
    (T^a)_{ij} (T^a)_kl = T_F \left( \delta_{il} \delta{kj} - \frac{1}{N_c} \delta_{ij} \delta_{kj} \right)\,.
\end{equation}
...
\begin{equation} \label{eq:gaugefixing}
    \mathcal{L}_{R_\xi} = -\frac{1}{2 \xi} \left(\partial_\mu A^a_\mu \right)^2\,,
\end{equation}
...
\begin{equation} \label{eq:ghosts}
    \mathcal{L}_\text{ghosts} = (\partial_\mu \bar{c}^a)(\delta^{ac} \partial_\mu + g_s f^{abc} A^b_\mu ) c^c\,.
\end{equation}
