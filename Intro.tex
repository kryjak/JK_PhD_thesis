...
The understanding of elementary particles and their interactions is undoubtedly one of the most fascinating quests of the scientific endeavour. Modern particle physics has proven remarkably successful at explaining phenomena which occur at size scales orders of magnitude smaller than what our brains evolved to comprehend. The Standard Model (SM), which is a collection of Quantum Field Theories (QFTs) that describe the subatomic world, allows us to rigorously test our ideas about how the Universe functions at the most bare, fundamental level. It relies on a constant interplay between theoretical predictions and their experimental verification (or negation). From the perspective of a theoretical physicist, it is crucial to be able to derive increasingly precise predictions for physically observable quantities that can be measured in particle colliders such as the Large Hadron Collider (LHC) at CERN. This theoretical precision should match the precision that can be achieved experimentally. The growing amount of data from Run 3 at the LHC makes this a challenge for the theoretical community, pushing us to constantly improve our computational tools and refine our predictions. 

The subject of this thesis is precisely the story of trying to overcome current limitations in an effort to exploit the predictive power of QFT to an even greater extent. It is a famous fact of life (at least to a rather narrow group of people known as particle physicists) that QFT computations cannot be performed exactly, apart from the most trivial cases. Instead, one follows an approximate description through so-called perturbation theory. In this approach, the leading-order (LO) terms in the computation are low in number and complexity. They represent a crude, almost 'back-of-the-envelope' estimation of the full answer. The next-to-leading-order (NLO) terms increase in number and are harder to calculate. Nonetheless, they provide a refinement of the NLO result and are absolutely necessary for precision phenomenology. Roughly speaking, the cutting edge of QFT computations is currently at the next-to-next-to-leading-order (NNLO), which can involve millions of individual terms, each of which can be incredibly hard to evaluate. In this thesis, we will focus on the description of one of the key ingredients needed for such computations, i.e. the scattering amplitudes. We hope to provide the reader with detailed understanding of why they are important in the first place, how they are calculated and the difficulties that we have to overcome in practice. 

Naturally, we cannot jump into full detail straightaway. We hope that this thesis will be approachable to someone who already possesses basic knowledge of QFT, but we certainly do not assume previous experience in this topic at research level. For this reason, we dedicate the rest of Chapter~\ref{sec:intro} to reviewing the rudimentary information needed further on. This content should be familiar from a thorough introductory course to QFT. Next, in Chapter~\ref{sec:tools} we introduce more advanced and specialised techniques that are used to compute scattering amplitudes. We hope that despite the breadth of the concepts covered and often heavy mathematical detail, this chapter will not be too hard to follow. Our reward will come in Chapters~\ref{sec:Hbb} and ~\ref{sec:Wyj}, where we will use the techniques described thus far to compute scattering amplitudes necessary for the NNLO description of two selected processes at the LHC.
In particular, we will focus on amplitudes in Quantum Chromodynamics (QCD) and which are relevant to processes that have been deemed of high priority by the community. The biannual Les Houches report provides a wishlist of processes whose improved theoretical description would greatly contribute to our understanding of fundamental particles~\cite{Huss:2022ful}. Amongst others, two-loop QCD scattering amplitudes relevant to Higgs boson production in association with a bottom-quark pair, $\ppbbh$, as well as $W^\pm$ boson production with a photon and a jet, $\ppWgj$, are highly desired. Our results represent some of the first results for five-point processes with an external massive leg at this loop order.

In Chapter~\ref{sec:selfdual}, we take a small detour and explore the scattering amplitudes for the interaction of the Higgs boson with multiple gluons, as studied through an effective interaction. The techniques employed in this chapter are closely related to, yet not a part of, our main workflow utilised in Chapters~\ref{sec:Hbb} and\ref{sec:Wyj}. We will show how even if our standard approach proves unsuccessful in the face of a prohibitively difficult computation, we can still extract a lot of meaningful information about the amplitudes through these complementary techniques. Finally, in Chapter~\ref{sec:conclusions}, we draw our conclusions and discuss directions for future work.

\section{QFT background}
The rest of this chapter is meant to provide the reader with a bridge between elementary knowledge of QFT and state-of-the-art techniques that are used to extract predictions from it. We start by providing a brief summary of the relevant information about QFTs, and in particular QCD (Sec.~\ref{sec:QCD}. Next, in Sec.~\ref{sec:particlescattering} we describe how to connect this theory to physical observables that can be measured experimentally. This introduces the notion of a scattering amplitude (Sec.~\ref{sec:fromcrosssectoamps}, which will be the main focus of this thesis. Finally, in Sec.~\ref{sec:scatteringamps} we discuss the famous problem of divergences present in QFT computations and show how in the process of trying to keep them under control, we discover a surprising feature of the QCD interactions. At the end, we will be ready to tackle the more advanced topics presented in Chapter~\ref{sec:tools}.
...
\end{equation}
which in the Weyl representation of the $\gamma$-matrices is just:
\begin{equation}
    \gamma^5 = 
    \begin{pmatrix}
        -\mathds{1} & 0 \\
...
\begin{equation} \label{eq:completeness:bosons}
    \sum_{s=-,+} \eps_\mu^s(p, q) (\eps_\nu^s(p, q))^{\ast} = -g_{\mu\nu} + \frac{p_\mu q_\nu + p_\nu q_\mu}{p \cdot q} \,,
\end{equation}
where $q^\mu \neq p^\nu$ is an arbitrary reference vector which is introduced to help decompose the four-component vectors $\varepsilon^\mu$ into two-component objects, similarly to the decomposition of the Dirac spinor into Weyl spinors. The end results of computations are independent of $q^\mu$, i.e. the reference vector is consistent with gauge invariance. 
...
\end{equation}
is the covariant derivative introduced to ensure gauge invariance under SU($N_c$) transformations\footnote{We will try to keep the degree $N_c$ of the group SU($N_c$) generic in the discussion below, but of course we are interested in the QCD case of $N_c=3$.}.

The QCD Lagrangian contains three types of fields. Quarks and antiquarks, corresponding to $\overline{\psi}_q^{\,i}$ and $\psi_q^i$, respectively, transform in the fundamental and antifundamental representations of SU($N_c$). In QCD, they have $n_f = 6$ flavours. The indices $i$ denote the colour of the (anti)quarks and take values $\{1,\ldots,N_c\}$. It is common to drop the colour index and work with $\psi_q$, which is understood as $\psi_q \equiv \left(\psi_q^r, \psi_q^b, \psi_q^g \right)^T$. The fundamental fields are acted on by the $N_c^2-1$ fundamental generators of SU($N_c$) with dimensions $(N_c \times N_c)$. For SU(3), they are given by:
\begin{equation}
...
\end{equation}
with $T_F$ typically $\frac{1}{2}$ or $1$. Finally, let us list one more extremely useful relation satisfied by the fundamental generators:
\begin{equation} \label{eq:fierzcolour}
    (T^a)_{ij} (T^a)_{kl} = T_F \left( \delta_{il} \delta{kj} - \frac{1}{N_c} \delta_{ij} \delta_{kj} \right)\,.
\end{equation}
...
\end{equation}
where $a,b,c \in \{1, \ldots, N_c^2-1\}$ and the totally antisymmetric objects $f^{abc}$ are known as the structure constants. Typically, the adjoint generators are normalised such that $\tr (T_\text{adj}^aT_\text{adj}^b) = N_c \delta^{ab}$. 

SU($N_c$) generators in any representation (let us denote them $T_R^a$) satisfy the following defining relation of the Lie algebra:
\begin{equation} \label{eq:liealgebra}
    \left[T^a, T^b\right] = \ii f^{abc} T^c\,.
\end{equation}
Note that for Abelian groups, the structure constants vanish. For the non-Abelian SU(3) group of QCD, they are non-vanishing, however, and it is useful to invert the above relation in terms of the fundamental generators:
\begin{equation} \label{eq:fabc}
    f^{abc} = -\frac{\ii}{T_F} \tr \left(\left[T^a, T^b \right] T^c \right)\,.
\end{equation}
Thus, the structure constants, which are associated with 3- and 4-gluon vertices in Feynman diagrams, can be written in terms of traces of fundamental generators. This fact, along with Eq.~\ref{eq:fierzcolour}, becomes very useful when computing scattering amplitudes involving gluons, as we will see in Sec.~\ref{sec:colourdec}.

In the process of deriving the gluon propagator, the notion of gauge invariance leads to two important qualitative features. First of all, just as in the case of the photon propagator in QED, instead of explicitly fixing the gauge, we find it convenient to introduce an auxiliary (i.e. non-propagating) field $\xi$. We write:
...
\section{Scattering amplitudes} \label{sec:scatteringamps}
...
\begin{figure}[t]
	...
	\begin{tikzpicture}[scale=0.5]
        ...
        \node (text) [text width=5cm] at (12, 0.5) {\begin{equation*}
            \longleftrightarrow \qquad \prod_{l=1}^L \left( \int \frac{\dd^d k_l}{\ii \, \pi^{d/2}}\right) \frac{N}{\prod_i D_i(k,p,m)}
        \end{equation*}};
        \end{feynman}
	\end{tikzpicture}
    \caption{An $L$-loop Feynman diagram corresponds to an $L$-fold Feynman integral. We continue the integrals from $4$ to $d$ dimensions (see below for explanation). The integrand is determined by the Feynman rules of the underlying theory. \JK{Should I explain what $N$ and $D$ are?} \JK{Should I include $\mu^{2\eps}$ and the symmetry factor}? }
...
\end{equation}
where the arbitrary regularisation scale $\mu$ is needed to keep the mass dimension of the coupling constant fixed. Then, the loop integrals are a function of $\eps$ and the UV/IR divergences appear as poles in the Laurent expansion in $\eps$. Any cancellations between terms can now also be tracked through this dimensional regulator. Typically, when computing scattering amplitudes we are interested only in terms up to $\order{\eps^0}$ since the four-dimensional result is recovered by setting $\epsilon = 0$ at the end of the computation\footnote{A common exception to this rule is when an $(l<L)$-loop amplitude is meant to be multiplied by the pole operator mentioned above in order to to derive the pole structure of an $L$-loop amplitude. Then terms beyond $\order{\eps^0}$ are required, usually up to $\order{\eps^2}$}.
