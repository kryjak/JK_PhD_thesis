...
This chapter is meant to provide the reader with a bridge between elementary knowledge of QFT and state-of-the-art techniques that are used to extract predictions from these QFTs. We start by providing a brief summary of the relevant information about QFTs, and in particular QCD. Next, we describe how to connect this theory to physical observables that can be measured experimentally. This introduces the notion of a scattering amplitude, which will be the main focus of this thesis. Finally, we discuss the famous problem of divergences present in QFT computations and show how in the process of trying to keep them under control, we discover a surprising feature of the QCD interactions. At the end, we will be ready to tackle the more advanced topics presented in Chapter~\ref{sec:tools}.
...
Apart from fermions, we also need to know how to deal with bosons. In particular, spin\=/1 bosons are described by vector fields $A^\mu$, which transform in the $(\frac{1}{2}, \frac{1}{2})$ representation of the Lorentz group. For the massive gauge bosons, that is $w^\pm$ and $Z^0$, imposing their equations of motions leads to three distinct solutions labelled as $\varepsilon^\mu_i(p)$ and referred to as polarisation vectors. On the other hand, massless gauge bosons, i.e. photons and gluons, have only two polarisations, which can be interpreted as helicity eigenstates. Similarly to the completeness relations for spin\=/1/2 fermions, Eq.~\ref{eq:completeness:fermions}, the polarisation vectors of \textit{massless} gauge bosons satisfy the following property:
\begin{equation} \label{eq:completeness:bosons}
    \sum_{s=\pm} \eps_\mu^s(p, q) (\eps_\nu^s(p, q))^{\ast} = -g_{\mu\nu} + \frac{p_\mu q_\nu + p_\nu q_\mu}{p \cdot q} \,,
\end{equation}
\JK{where $q$ is} \JK{is this true only in the axial gauge? See Eq. 30 of `Calculating scattering amplitudes efficiently'. We don't use the axial gauge.}
\begin{equation}
    \JK{Massive \; spin\; sum}
\end{equation}
Finally, \JK{Higgs}
