...
\Cref{app:poles} is devoted to the UV renormalisation and IR factorisation which determine the pole structure of the amplitudes.
...
Since the MIs satisfy canonical DEs~\labelcref{eqQED:canonicalDEs}, the $\eps$-order of the \ac{MI} coefficients $\vec{g}^{(w)}_{\tau}(\vec{s})$ equals their \emph{transcendental weight}~\cite{Henn:2013pwa}.
...
In order to solve the DEs~\labelcref{eqQED:canonicalDEs} we need boundary values, i.e., values of all MIs up to order $\eps^4$ at a phase-space point. 
...
The canonical DEs \labelcref{eqQED:canonicalDEs} and the boundary values for all integral families are the input for the algorithm of \incite{Abreu:2023rco} for constructing a basis of special functions. We refer to the original work for a thorough discussion. 
