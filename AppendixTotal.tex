...
\label{app:oneloop}
In this Appendix, we collect the numerical results relevant to the amplitudes presented in Chapters~\ref{sec:Hbb} and \ref{sec:Wyj}. Tables~\ref{tab:benchmarkbare1L} and \ref{Wyjtab:benchmark1Lnf0bare} show the bare one-loop amplitudes (normalised by the tree-level amplitudes) for the two processes. Tables~\ref{tab:benchmarkfinremsq1L} and ~\ref{Wyjtab:benchmarkfinremsq1L} show the corresponding reduced squared tree-level amplitudes $\cH^{(0)}$ and reduced squared one-loop finite remainders (normalised by $\cH^{(0)}$).
\begin{table}[t!]
\centering
